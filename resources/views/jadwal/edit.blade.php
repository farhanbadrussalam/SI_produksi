...
<div class="card shadow mb-4 col-md-9 col-sm-12 p-0">
    <div class="card-header">
        <h6 class="m-0 font-weight-bold text-primary">
            <a href="{{ url('jadwal') }}" class="mr-3"><i class="fas fa-arrow-left"></i></a>Edit jadwal
        </h6>
    </div>
    <div class="card-body">
        <form action="{{ url('jadwal/'.$jadwal->id) }}" method="post">
            ...
            <div class="form-row mb-2">
                <div class="col-md-6">
                    ...
                    <select name="operator" id="operator" class="selectpicker show-tick form-control @error('operator') is-invalid @enderror" data-live-search="true" title="Select Operator">
                        ...
                        @if(old('operator', $jadwal->user_id) == $value->id)
                ...
                <div class="col-md-6">
                    ...
                    <select name="mesin" id="mesin" onchange="selectMesin(this)" class="selectpicker show-tick form-control @error('mesin') is-invalid @enderror" data-live-search="true" title="Select Mesin">
                        ...
                        @if($jadwal->mesin_id == $key->id)
                        <option value="{{ $key->id }}" selected>{{ $key->nama_mesin }}</option>
                        @else
            ...
            <div class="form-row mb-4">
                <div class="col-md-6">
                    ...
                    <select name="proses_mesin[]" id="proses_mesin" class="selectpicker show-tick form-control @error('proses_mesin') is-invalid @enderror" multiple data-live-search="true" title="Proses mesin">
                    </select>
                    @error('proses_mesin')
                    <div class="invalid-feedback">
                        {{ $message }}
                ...
                <div class="col-md-3">
                    ...
                    <input type="time" value="{{ old('mulai', $jadwal->mulai) }}" class="form-control @error('mulai') is-invalid @enderror" name="mulai" id="mulai">
                    @error('mulai')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <div class="col-md-3">
                    ...
                    <input type="time" value="{{ old('selesai', $jadwal->selesai) }}" class="form-control @error('selesai') is-invalid @enderror" name="selesai" id="selesai">
                    @error('selesai')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
...
<script>
    $(document).ready(function() {
        selectMesin(document.getElementById('mesin'), '<?= $jadwal->proses ?>');
    })

    function selectMesin(obj, jadwalProses = false) {
        if (jadwalProses) {
            jadwalProses = JSON.parse(jadwalProses);
        }
        ...
        for (const row of dataProses) {
            jadwalProses.includes(`${row.id}`) && arrSelect.push(row.nama_proses);
        ...
        $('#proses_mesin').selectpicker('val', jadwalProses);
    }
</script>
@endsection